...
export default function ForgotPassword() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(data.msg || "Gagal mengirim kode reset password");
      }

      setSuccess(data.msg);

      // Redirect ke halaman reset-password dengan query email setelah delay 2 detik
      setTimeout(() => {
        router.push({
  ...
  return (
    <>
      <Head>
        <title>Lupa Password - Museum Lampung</title>
        <meta name="description" content="Lupa password akun Museum Lampung" />
      </Head>
